...
import { IBaseRepository, IBrowsingRepository, IQueryRepository } from "@shared/repositories";
import {
...
export default class RegisterHandler implements ICommandHandler<RegisterCommand> {
  constructor(
    ...
    private readonly catalog_view_repository: IBrowsingRepository<ProductCatalogView>,
  ...
  async execute({
    ...
    const product_entity = await this.entity_manager.transaction(async (manager) => {
...

      {
        ...
        await this.catalog_query_repository.create(catalog);
      }
